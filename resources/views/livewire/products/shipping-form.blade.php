<div class="grid grid-cols-4 bg-white" shipping-id="{{ $shipping['id'] }}" x-data>
    ...
    <div class="flex items-center pr-6">
        <x-input.base-price class="text-gray-600 bg-transparent outline-none focus:outline-none p-0 border-0 focus:ring-0" wire:model='shipping.price' placeholder="$ Price" value="{{ $shipping['standalone_price'] }}" />
    </div>
    
    <div class="grid grid-cols-4 items-center justify-center">
        <div class="col-span-3">
            <x-input.base-price class="text-gray-600 bg-transparent outline-none focus:outline-none p-0 border-0 focus:ring-0" placeholder="Quantity" value="{{ $shipping['withothers_price'] }}" />
        </div>
